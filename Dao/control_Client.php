...
{
    $sql = "SELECT * FROM `commodity` WHERE `name_commodity` = ?"; // Sử dụng prepared statement với placeholder
    return pdo_check_data($sql, [$name]); // Gọi hàm pdo_check_data để kiểm tra
}
...
function select_client_by_role($role)
{
    $sql = "SELECT * FROM client WHERE id_client= " . $role;
    return pdo_query_one($sql);
}
